...
@section('content')
    <div class="tf-wrapper">
        ...
        <form method="POST" action="{{ route('track.calculate', $user->id) }}" class="tf-form" id="tfForm">
            @csrf
                ...
                @foreach ($subjects as $s)
                    <div class="tf-subject-card" style="--accent: {{ $s['color'] }}">
                        ...
                        <div class="tf-input-wrap">
                            <input type="number" name="{{ $s['key'] }}" id="{{ $s['key'] }}" min="0"
                                max="{{ $s['max'] }}" value="{{ old($s['key']) }}" placeholder="0" class="tf-input"
                                required oninput="updateBar(this, {{ $s['max'] }})">
                            <span class="tf-input-max">/{{ $s['max'] }}</span>
                        </div>
                        <div class="tf-progress-bar">
                            <div class="tf-progress-fill" id="bar_{{ $s['key'] }}"
