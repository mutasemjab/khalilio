{{-- resources/views/user/quiz-landing.blade.php --}}
...
<div class="ql-wrapper">
    ...
    <div class="ql-header">
        <a href="{{ route('hub') }}" class="ql-back">← {{ app()->getLocale() == 'ar' ? 'العودة' : 'Back' }}</a>
        <div class="ql-icon">🧮</div>
        <h1 class="ql-title">
            {{ app()->getLocale() == 'ar' ? 'كيف تعرف مستواك بالرياضيات' : 'How to Know Your Math Level' }}
        </h1>
        <p class="ql-subtitle">
            {{ app()->getLocale() == 'ar'
    ...
    <div class="ql-info-card">
        <div class="ql-info-header">
            <h2 class="ql-info-name">{{ $quiz->name }}</h2>
            @if($quiz->description)
                <p class="ql-info-desc">{{ $quiz->description }}</p>
            @endif
        </div>
        <div class="ql-info-stats">
            <div class="ql-stat">
                <span class="ql-stat-icon">⏱️</span>
                <span class="ql-stat-val">{{ $quiz->duration_minutes }}</span>
                <span class="ql-stat-label">{{ app()->getLocale() == 'ar' ? 'دقيقة' : 'min' }}</span>
            </div>
            <div class="ql-stat">
                <span class="ql-stat-icon">❓</span>
                <span class="ql-stat-val">{{ $quiz->questions->count() }}</span>
                <span class="ql-stat-label">{{ app()->getLocale() == 'ar' ? 'سؤال' : 'questions' }}</span>
            </div>
            <div class="ql-stat">
                <span class="ql-stat-icon">⭐</span>
                <span class="ql-stat-val">{{ $quiz->total_marks }}</span>
                <span class="ql-stat-label">{{ app()->getLocale() == 'ar' ? 'علامة' : 'marks' }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="ql-groups">
        <h3 class="ql-groups-title">
            {{ app()->getLocale() == 'ar' ? 'المجموعات الدراسية' : 'Study Groups' }}
        </h3>
        <div class="ql-groups-grid">
            <div class="ql-group ql-group--a">
                <div class="ql-group-icon">🏆</div>
                <div class="ql-group-name">{{ app()->getLocale() == 'ar' ? 'المجموعة A' : 'Group A' }}</div>
                <div class="ql-group-range">{{ app()->getLocale() == 'ar' ? '≥ 83%' : '≥ 83%' }}</div>
                <div class="ql-group-desc">{{ app()->getLocale() == 'ar' ? 'المستوى المتقدم' : 'Advanced' }}</div>
            </div>
            <div class="ql-group ql-group--b">
                <div class="ql-group-icon">⭐</div>
                <div class="ql-group-name">{{ app()->getLocale() == 'ar' ? 'المجموعة B' : 'Group B' }}</div>
                <div class="ql-group-range">{{ app()->getLocale() == 'ar' ? '60% - 82%' : '60% - 82%' }}</div>
                <div class="ql-group-desc">{{ app()->getLocale() == 'ar' ? 'المستوى المتوسط' : 'Intermediate' }}</div>
            </div>
            <div class="ql-group ql-group--c">
                <div class="ql-group-icon">📚</div>
                <div class="ql-group-name">{{ app()->getLocale() == 'ar' ? 'المجموعة C' : 'Group C' }}</div>
                <div class="ql-group-range">{{ app()->getLocale() == 'ar' ? 'أقل من 60%' : 'Below 60%' }}</div>
                <div class="ql-group-desc">{{ app()->getLocale() == 'ar' ? 'يحتاج تعزيز' : 'Needs support' }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="ql-cta">
        ...
        <h3>{{ app()->getLocale() == 'ar' ? 'جاهز للبدء؟' : 'Ready to start?' }}</h3>
        <p>{{ app()->getLocale() == 'ar'
            ? 'سجّل بياناتك أولاً ثم ابدأ الامتحان مباشرة'
            : 'Register your details first, then start the quiz immediately' }}</p>
        <a href="{{ route('student.form', ['redirect_to' => 'quiz']) }}" class="ql-cta-btn">
            <span>🚀</span>
            {{ app()->getLocale() == 'ar' ? 'سجّل وابدأ الامتحان' : 'Register & Start Quiz' }}
        </a>
    </div>
    ...
    <div class="ql-empty">
        ...
        <h3>{{ app()->getLocale() == 'ar' ? 'لا يوجد امتحان متاح حالياً' : 'No quiz available yet' }}</h3>
        <p>{{ app()->getLocale() == 'ar' ? 'سيتم إضافة الامتحان قريباً' : 'A quiz will be added soon' }}</p>
        <a href="https://whatsapp.com/channel/0029Vb35e8I2v1Ik7V9Khs3r"
           target="_blank" class="ql-wa-btn">📱 {{ app()->getLocale() == 'ar' ? 'انضم للقناة' : 'Join Channel' }}</a>
    </div>
