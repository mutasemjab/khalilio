{{-- resources/views/user/pdf-bag.blade.php --}}
@extends('layouts.front')

@section('title', 'الحقيبة الإلكترونية - خليليو')
...
@section('content')
    <div class="pb-wrapper">
                ...
                @foreach ($files as $file)
                    <a href="{{ $file['url'] }}" target="_blank" class="pb-card">
                        ...
                        <div class="pb-card__body">
                            <h3 class="pb-card__name">{{ $file['name'] }}</h3>
                            <div class="pb-card__meta">
                                <span class="pb-badge">{{ $file['size'] }}</span>
                                <span class="pb-badge pb-badge--date">{{ $file['date'] }}</span>
                            </div>
                        </div>
        ...
        @else
            <div class="pb-empty">
                ...
                <h3>{{ app()->getLocale() === 'ar' ? 'لا توجد ملفات بعد' : 'No files yet' }}</h3>
                <p>{{ app()->getLocale() === 'ar' ? 'سيتم إضافة ملفات PDF قريباً، تابعنا على واتساب.' : 'PDF files will be added soon. Follow us on WhatsApp.' }}</p>
    ...
    <style>
        ...
        .pb-header {
            text-align: center;
            margin-bottom: 32px;
            animation: pbFadeDown 0.7s ease-out;
            position: relative;
        }
        ...
        .pb-icon {
            ...
            display: block;
            margin-bottom: 12px;
            animation: pbFloat 3s ease-in-out infinite;
        }
        ...
        .pb-card__pdf-icon {
            font-size: 42px;
            filter: drop-shadow(0 3px 6px rgba(17, 153, 142, 0.25));
        }
        ...
        .pb-card:hover .pb-card__download {
        ...
        .pb-empty {
            text-align: center;
            padding: 60px 20px;
            animation: pbFadeDown 0.6s ease-out;
        }
