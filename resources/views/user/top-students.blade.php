{{-- resources/views/user/top-students.blade.php --}}
@extends('layouts.front')

@section('title', 'الأوائل - خليليو')
...
<div class="ts-wrapper">
    ...
    <div class="ts-grid">
        ...
        <div class="ts-card" style="animation-delay: {{ $i * 0.12 }}s">

            {{-- Rank badge --}}
            @if($student->rank)
            <div class="ts-rank">
                @if($student->rank == '1' || $student->rank == 'الأول')🥇
                @elseif($student->rank == '2' || $student->rank == 'الثاني')🥈
                @elseif($student->rank == '3' || $student->rank == 'الثالث')🥉
                @else
                <span class="ts-rank-num">{{ $student->rank }}</span>
                @endif
            </div>
            @endif
            ...
            <div class="ts-photos">
                ...
                <div class="ts-photo-main">
                    @if($student->photo)
                        <img src="{{ asset('assets/admin/uploads/' . $student->photo) }}" alt="{{ $student->name }}" class="ts-photo-img">
                    @else
                ...
                <div class="ts-photo-thumbs">
                    <div class="ts-thumb" onclick="openPhotoModal('{{ $student->grades_photo ? asset('assets/admin/uploads/'.$student->grades_photo) : '' }}', 'كشف العلامات')">
                        @if($student->grades_photo)
                            <img src="{{ asset('assets/admin/uploads/' . $student->grades_photo) }}" alt="كشف العلامات">
                        @else
                    ...
                    <div class="ts-thumb" onclick="openPhotoModal('{{ $student->certificate_photo ? asset('assets/admin/uploads/'.$student->certificate_photo) : '' }}', 'الشهادة')">
                        @if($student->certificate_photo)
                            <img src="{{ asset('assets/admin/uploads/' . $student->certificate_photo) }}" alt="الشهادة">
                        @else
                            <div class="ts-thumb-placeholder">🏅</div>
                        @endif
            ...
            {{-- Info --}}
            <div class="ts-info">
                <h3 class="ts-name">{{ $student->name }}</h3>
                @if($student->school_name)
                <p class="ts-school">🏫 {{ $student->school_name }}</p>
                @endif
                @if($student->average)
                <div class="ts-avg">
            ...
            <div class="ts-card-sparkles">
                @for($j=0;$j<5;$j++)
                <span class="ts-card-star" style="--x:{{ rand(5,95) }}%;--d:{{ rand(2,5) }}s;--delay:{{ $j*0.4 }}s">✦</span>
                @endfor
            </div>
        </div>
        @endforeach
