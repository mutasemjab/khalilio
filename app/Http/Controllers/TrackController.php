...
class TrackController extends Controller
{
    ...
    public function calculate(Request $request, User $user)
    {
        $request->validate([
            ...
            'arts'      => 'required|integer|min:0|max:100',
            'pe'        => 'required|integer|min:0|max:100',
            'digital'   => 'required|integer|min:0|max:100',
        ...
        $subjects = [
            ...
            'arts'      => ['label' => 'التربية الفنية والموسيقية والمسرحية', 'max' => 100, 'value' => (int)$request->arts],
            'pe'        => ['label' => 'التربية الرياضية',                    'max' => 100, 'value' => (int)$request->pe],
            'digital'   => ['label' => 'المهارات الرقمية',                    'max' => 100, 'value' => (int)$request->digital],
            'financial' => ['label' => 'الثقافة المالية',                     'max' => 100, 'value' => (int)$request->financial],
        ];

        $totalScore = array_sum(array_column($subjects, 'value'));
        $totalMax   = array_sum(array_column($subjects, 'max')); // 1900
